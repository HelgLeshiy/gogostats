...
from messages_pb2 import (DragonDmgList, DragonDmgRequest, GuildHeaderRequest,
                          GuildInfo, GuildListRequest, GuildMemberList,
                          GuildPlayerHeader)
...
def parse_dragon_dmg_list(message: bytes) -> DragonDmgList:
    ...
    for dragon_player in dragon_dmg_list.list:
        send_payload["players"].append(
            {"id": dragon_player.header.id, "dmg": dragon_player.damage}
        )
        logging.info(
...
def send_command(
    s: socket.socket, character_idx: int, shift: int, command: bytes, payload: bytes
):
    checksum = zlib.crc32(payload)
    sequence = bytes.fromhex("00 00 06 00") + (
        128 + shift + character_idx - 1
    ).to_bytes(1, byteorder="big")
    total = (
        command
        + bytes.fromhex("000000")
        + character_idx.to_bytes(1, byteorder="big")
        + bytes.fromhex("0000")
        + sequence
...
def get_guild_header(
    s: socket.socket, character_idx: int, player_id: int, strange_key: str, token: str
):
    guild_header_request = GuildHeaderRequest(
        token=token, playerId=player_id, strangeKey=strange_key
    )
...
def get_guild_list(
    ...
    guild_list_request = GuildListRequest(
        token=token,
        playerId=player_id,
        strangeKey=strange_key,
        offset=offset,
        count=count,
    )
...
def get_dragon_dmg_list(
    ...
    dragon_dmg_request = DragonDmgRequest(
        playerId=player_id,
        userId=user_id,
        strangeKey=strange_key,
        var_910007=910007,
...
while True:
    if COLLECT_DMG == "false":
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            guild_header = get_guild_header(
                s, CHARACTER_IDX, CHARACTER_ID, STRANGE_KEY, USER_ID
            )
