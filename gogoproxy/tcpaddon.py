...
class SimpleTCPInterceptor:
    def tcp_start(self, flow: tcp.TCPFlow):
        ...
        logging.info(f"TCP connection started: {flow.client_conn.peername} -> {flow.server_conn.peername}")
    ...
    def tcp_message(self, flow: tcp.TCPFlow):
        if flow.server_conn.address and flow.server_conn.address[1] == 9190:
            ...
            if message.content.find(bytes.fromhex('1c020000000100000000')) != -1:
                ...
                payload_start = message.content.find(bytes.fromhex('1c020000000100000000')) + 13  # Skip the length prefix
                ...
                for member in guild_info.list.guildPlayers:
                    ...
                    logging.info(f"Player Name: {member.header.name}, Level: {member.header.lvl}, Class: {member.header.player_class}, Stage: {member.stage}, Power: {member.power}")
                _ = post_json('http://stream1.transcriptic.ru:5997/v1/players/add', payload=payload)
            ...
            if message.content.find(bytes.fromhex('1c030000000100000000')) != -1:
                ...
                payload_start = message.content.find(bytes.fromhex('1c030000000100000000')) + 13  # Skip the length prefix
                payload = message.content[payload_start:]
            ...
            if message.content.find(bytes.fromhex('0c0f0000000100000000')) != -1:
                ...
                payload_start = message.content.find(bytes.fromhex('0c0f0000000100000000')) + 11  # Skip the length prefix
                payload = message.content[payload_start:]
                ...
                logging.info(f"Player Name: {player.name}, Level: {player.lvl}, Class: {player.player_class}, Stage: {player.stage}, Power: {player.power}")
                _ = post_json('http://stream1.transcriptic.ru:5997/v1/players/add', payload=send_payload)
        # Example: Modify the message content (e.g., replace a specific byte sequence)
        # if message.from_client and b"original" in message.content:
        #     message.content = message.content.replace(b"original", b"modified")

    def tcp_end(self, flow: tcp.TCPFlow):
        """A TCP connection has ended."""
        logging.info(f"TCP connection ended: {flow.client_conn.peername} -> {flow.server_conn.peername}")
